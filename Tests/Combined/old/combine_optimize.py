...
# permutation test loop 10 simulations
# perm_test_results = []
# for i in range(3):
#     print(i)
...
#               y=new_df_vc.values, mode='lines+markers'))
...
for i in range(10):
    ...
    for index, row in df_grouped:
        ...
        weights = temp_weighted_officers.loc[temp_weighted_officers['Beat']
                                             == beat, 'Weights']
        # count number of 0 weights in weights
        num_officers = num_officers - len(weights[weights == 0])
        ...
        # add officers to new dataframe, use pd.concat to add the OfficerID and Beat
        weighted_df = pd.concat(
            [weighted_df, officers_sample[['OfficerID', 'Beat']]])
        # officers_sample.apply(lambda x: weighted_df.append(
        #     pd.DataFrame([[x['OfficerID'], x['Beat']]], columns=['OfficerID', 'Beat'])), axis=1)
        # update officer occurences in weighted_officers
        temp_weighted_officers.loc[temp_weighted_officers['OfficerID'].isin(
            officers_sample['OfficerID']), 'Occurences'] += 1
        # update officer weights in weighted_officers
        temp_weighted_officers.loc[temp_weighted_officers['OfficerID'].isin(
            officers_sample['OfficerID']), 'Weights'] += alpha
        # if number of occurences > 35, make weights = 0
        temp_weighted_officers.loc[temp_weighted_officers['Occurences'] >
                                   35, 'Weights'] = 0
...
fig.data[3].name = 'Original Results'
